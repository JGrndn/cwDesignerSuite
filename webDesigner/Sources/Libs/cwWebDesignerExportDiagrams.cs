...
namespace Casewise.webDesigner.Libs
{
    ...
    internal class cwWebDesignerExportDiagrams
    {
        ...
        public class PaletteEntryCondition
        {
        ...
        {
            public PaletteEntryRegionJSON() { }
            public PaletteEntryRegionJSON(PaletteEntryRegionJSON clone)
            {
                this.rule = clone.rule;
                this.style = clone.style;
                this.symbol = clone.symbol;
                this.picturesId = clone.picturesId;
                this.pictureID = clone.pictureID;
                this.picturesIdByPropertyValue = clone.picturesIdByPropertyValue;
                this.propertyScriptName = clone.propertyScriptName;
                this.horizontalAlignment = clone.horizontalAlignment;
                this.verticalAlignment = clone.verticalAlignment;
                this.textOrientation = clone.textOrientation;
                this.textRotation = clone.textRotation;
                this.type = clone.type;
                this.associationType = clone.associationType;
                this.leftType = clone.leftType;
                this.topType = clone.topType;
                this.widthType = clone.widthType;
                this.heightType = clone.heightType;
                this.x = clone.x;
                this.y = clone.y;
                this.w = clone.w;
                this.h = clone.h;
                this.labelValue = clone.labelValue;
                this.anchor = clone.anchor;
            }

            public PaletteEntryCondition rule { get; set; }
        ...
        private PaletteEntryRegionJSON createDefaultExplosionRegion(PaletteEntry pe)
        {
            int SIZE = 5;
            PaletteEntryRegionJSON json = new PaletteEntryRegionJSON();
            cwWebDesignerStyle style = new cwWebDesignerStyle();
            json.style = style.getJSONObject();
            json.symbol = 0;
            json.pictureID = -1;
            json.picturesId = new List<int>();
            json.type = "explosion";
            json.anchor = "bottom";
            json.topType = "%";
            json.leftType = "%";
        ...
        private void exportPaletteEntryRegions(StreamWriter json_file, PaletteEntry pe, int templateID)
        {
            ...
            foreach (PaletteEntry.PaletteShapeRegion paletteShapeRegion in activeRegions)
            {
                ...
                if (regionType.Equals(RegionRepresentationType.Navigation))
                {
                    type = "navigation";
                    peRegionJSON.associationType = "ANYOBJECTSHOWNASSHAPEINDIAGRAM";
                }
                ...
                peRegionJSON.textOrientation = paletteShapeRegion.Orientation.ToString();
                peRegionJSON.textRotation = paletteShapeRegion.TextRotation.ToString();
                ...
                switch (paletteShapeRegion.Anchor)
                {
                    case RegionDefs.Anchor.Bottom:
                        peRegionJSON.anchor = RegionDefs.Anchor.Bottom.ToString().ToLower();
                        break;
                    case RegionDefs.Anchor.Left:
                        peRegionJSON.anchor = RegionDefs.Anchor.Left.ToString().ToLower();
                        break;
                    case RegionDefs.Anchor.Right:
                        peRegionJSON.anchor = RegionDefs.Anchor.Right.ToString().ToLower();
                        break;
                    case RegionDefs.Anchor.Top:
                        peRegionJSON.anchor = RegionDefs.Anchor.Top.ToString().ToLower();
                        break;
                    default:
                        peRegionJSON.anchor = RegionDefs.Anchor.None.ToString().ToLower();
                        break;
                }
                ...
                if (dashboard.RegionRepresentation.CurrentType.Equals(RegionRepresentationType.VisualizationUsingReferenceProperty))
                {
                    foreach (var br in dashboard.BandingRows)
                    {
                        PaletteEntryRegionJSON conditionalPeRegionJson = new PaletteEntryRegionJSON(peRegionJSON);
                        if (br.Value.Style.Style != null)
                        {
                            cwWebDesignerStyle style = stylesByID[br.Value.Style.Style.Id];
                            conditionalPeRegionJson.style = style.getJSONObject();
                            int symbol = (int)br.Value.SymbolNumber;
                            conditionalPeRegionJson.symbol = symbol;
                            if (pictureIDBySymbolIDAndTemplateID[templateID].ContainsKey(symbol))
                            {
                                conditionalPeRegionJson.pictureID = pictureIDBySymbolIDAndTemplateID[templateID][symbol];
                                conditionalPeRegionJson.picturesId.Add(conditionalPeRegionJson.pictureID);
                                if (br.Value.HighValue != null)
                                {
                                    int tmpValue = 0;
                                    if (int.TryParse(br.Value.HighValue.ToString(), out tmpValue))
                                    {
                                        conditionalPeRegionJson.picturesIdByPropertyValue[tmpValue.ToString()] = conditionalPeRegionJson.pictureID;
                                    }
                                }
                            }
                        }
                        PaletteEntryCondition rule = new PaletteEntryCondition();
                        rule.operande = br.Value.Operation.GetOperator().ToString();
                        rule.minValue = (br.Value.LowValue != null) ? br.Value.LowValue.ToString() : string.Empty;
                        rule.maxValue = (br.Value.HighValue != null) ? br.Value.HighValue.ToString() : string.Empty;
                        rule.value = (br.Value.HighValue != null) ? br.Value.HighValue.ToString() : string.Empty;

                        conditionalPeRegionJson.rule = rule;
                        activeRegionsJSONs.Add(conditionalPeRegionJson);
                    }
                }
                else if (dashboard.RegionRepresentation.CurrentType.Equals(RegionRepresentationType.Association))
                {
                    foreach (var br in dashboard.BandingRows)
                    {
                        PaletteEntryRegionJSON conditionalPeRegionJson = new PaletteEntryRegionJSON(peRegionJSON);
            ...
            if (!hasExplosionRegion)
            {
                activeRegionsJSONs.Add(createDefaultExplosionRegion(pe));
            }
        ...
        private void exportPaletteEntry(StreamWriter json_file, PaletteEntry pe, int templateID)
        {
            ...
            json_file.Write("\"joinerToEndSymbol\":" + (int)pe.ToEnd + ",");
            json_file.Write("\"joinerFromEndSymbol\":" + (int)pe.FromEnd + ",");
            ...
            RegionDefs.HorizontalJustification horizontalJustification = pe.HorizontalJustification;
            RegionDefs.VerticalJustification verticalJustification = pe.VerticalJustification;
            ...
            json_file.Write("\"textOrientation\" : \"" + pe.Orientation.ToString() + "\", ");
            json_file.Write("\"textRotation\" : \"" + pe.TextRotation.ToString() + "\", ");
        ...
        private void exportJoiners(StreamWriter json_file, int _dID, cwLightDiagram d, Dictionary<int, List<cwJoiner>> _joinersByDiagram)
        {
            ...
            foreach (cwJoiner dj in joiners)
            {
                ...
                if (dj.nameZone.isValid())
                {
                    ...
                    s.Serialize(dj.nameZone, b);
                    json_file.Write("\"nameZone\" : " + b.ToString() + ", ");
                }

                json_file.Write("\"points\":[");
                int pNum = 0;
                List<Point> allPoints = dj.getPoints();
                int pCount = allPoints.Count();
        ...
        /// </summary>
        private void exportShapes(StreamWriter json_file, List<cwShape> _shapes, cwLightDiagram diagram, int templateId)
        {
            _shapes.Reverse();
            ...
            foreach (cwShape shape in _shapes)
            {
                ...
                {
                    if (pictureIDBySymbolIDAndTemplateID.ContainsKey(templateId) && pictureIDBySymbolIDAndTemplateID[templateId].ContainsKey(shape.customSymbol))
                    {
                        json_file.Write("\"customPicture\" : " + pictureIDBySymbolIDAndTemplateID[templateId][shape.customSymbol] + ", ");
                    }
                    else
                    {
                ...
                if (shape.lightObject != null)
                {
                    cwLightObjectJSON shapeObject = null;
                    if (shape.isInternalEvent())
                    {
                        // on veut les diagrammes qui montrent l'objet en tant qu'internal result
                        shapeObject = new cwLightObjectJSON(shape.lightObject, diagramLoader.getObjectGroupNode(currentLightModel, shape.lightObject.OTID), this.diagramLoader.getShapesManager().diagramIdsByInternalResult);
                    }
                    else if (shape.isInternalResult())
                    {
                        // on veut les diagrammes qui montrent l'objet en tant qu'internal event
                        shapeObject = new cwLightObjectJSON(shape.lightObject, diagramLoader.getObjectGroupNode(currentLightModel, shape.lightObject.OTID), this.diagramLoader.getShapesManager().diagramIdsByInternalEvent);
                    }
                    else
                    {
                        shapeObject = new cwLightObjectJSON(shape.lightObject, diagramLoader.getObjectGroupNode(currentLightModel, shape.lightObject.OTID));
                    }
        ...
        {
            // export image/picture/picture-1.png from the installation folder
        ...
        {
            Dictionary<string, string> diagramNameById = new Dictionary<string, string>();
            ...
            foreach (var diagram in diagramLoader.diagramsShapes)
            {
                ...
                diagramNameById[d.ID.ToString()] = d.properties["NAME"];
                ...
                Dictionary<int, string> pictureUuidBySymbolId = cwLightDiagram.GetPictureUuidFromSymbolId(d);
                foreach (var v in pictureUuidBySymbolId)
                {
                    if (!pictureIDBySymbolIDAndTemplateID[templateID].ContainsKey(v.Key))
                    {
                        if (picturesNode.usedOTLightObjectsByUUID.ContainsKey(v.Value))
                        {
                            pictureIDBySymbolIDAndTemplateID[templateID][v.Key] = picturesNode.usedOTLightObjectsByUUID[v.Value].ID;
                        }
                    }
                }

                exportShapes(json_file, diagramLoader.orderedShapes[diagram.Key], d, templateID);
                json_file.Write(",");
            ...
            JavaScriptSerializer index_serializer = new JavaScriptSerializer();
            StringBuilder index_output = new StringBuilder();
            index_serializer.Serialize(diagramNameById, index_output);
            StreamWriter index_json = cwWebDesignerTools.getStreamWriterErase(_fileManager.addToGeneratedPath("diagram" + "/json/" + "index_diagram" + "." + _fileManager.getJSONExtention()));
            index_json.Write(index_output);
            index_json.Flush();
            index_json.Close();
        }
        ...
        {
            //Casewise.webDesigner.Libs.Diagram.cwWebDesignerDiagramManager m = new Diagram.cwWebDesignerDiagramManager(currentLightModel);
            //m.DoLoad(_diagramsToLoad, propertiesToLoad);
